...
class InputHandler:
    ...
    def _create_toolbar(self) -> Toolbar:
        ...
        self._add_floor_button = toolbar.add_button(
            text=f"Add {self._current_floor_type.value}", on_click=self._on_add_floor_clicked, width=180, height=30
        )

        toolbar.add_button(text="Add Elevator Bank", on_click=self._on_add_elevator_bank_clicked, width=180, height=30)

        toolbar.add_button(text="Add Elevator Car", on_click=self._on_add_elevator_car_clicked, width=180, height=30)

        # Add Person button
        toolbar.add_button(text="Add Person", on_click=self._on_add_person_clicked, width=120, height=30)
    ...
    def _on_add_person_clicked(self, snapshot: BuildingSnapshot | None) -> None:
        ...
        start_horiz_pos: float = random.uniform(left_bounds, right_bounds)
