...
class TestElevatorSnapshot:
    ...
    def test_door_states(self) -> None:
        ...
        closed_snapshot = ElevatorSnapshot(
            id="elevator_1", vertical_position=Blocks(1.0), horizontal_position=Blocks(14.0),  # Wrap in Blocks
            destination_floor=1, elevator_state=ElevatorState.IDLE,
        ...
        open_snapshot = ElevatorSnapshot(
            id="elevator_2", vertical_position=Blocks(1.0), horizontal_position=Blocks(14.0),  # Wrap in Blocks
            destination_floor=1, elevator_state=ElevatorState.LOADING,
            nominal_direction=VerticalDirection.STATIONARY,
...
class TestBuildingSnapshot:
    ...
    def test_creation(self) -> None:
        ...
        person_snapshot = PersonSnapshot(
            person_id="person_1", current_floor_num=1, current_vertical_position=Blocks(1.0),  # Wrap in Blocks
            current_horizontal_position=Blocks(5.0), destination_floor_num=5, destination_horizontal_position=Blocks(10),  # Wrap in Blocks
            state=PersonState.WALKING, waiting_time=Time(0.0), mad_fraction=0.0,
            draw_color=(128, 128, 128)
        )
        
        elevator_snapshot = ElevatorSnapshot(
            id="elevator_1", vertical_position=Blocks(2.5), horizontal_position=Blocks(14.0),  # Wrap in Blocks
            destination_floor=5, elevator_state=ElevatorState.MOVING,
        ...
        floor_snapshot = FloorSnapshot(
            floor_type=FloorType.LOBBY, floor_number=1, floor_height=Blocks(1),  # Wrap in Blocks
            left_edge_block=Blocks(0), floor_width=Blocks(20), person_count=1,  # Wrap in Blocks
    ...
    def test_multiple_entities(self) -> None:
        ...
        floors: list[FloorSnapshot] = [
            FloorSnapshot(
                floor_type=FloorType.LOBBY, floor_number=1, floor_height=Blocks(1),  # Wrap in Blocks
                left_edge_block=Blocks(0), floor_width=Blocks(20), person_count=0,  # Wrap in Blocks
                floor_color=(200, 200, 200), floorboard_color=(10, 10, 10)
            ),
            FloorSnapshot(
                floor_type=FloorType.OFFICE, floor_number=2, floor_height=Blocks(1),  # Wrap in Blocks
                left_edge_block=Blocks(0), floor_width=Blocks(20), person_count=2,  # Wrap in Blocks
        ...
        elevators: list[ElevatorSnapshot] = [
            ElevatorSnapshot(
                id="elevator_1", vertical_position=Blocks(1.0), horizontal_position=Blocks(14.0),  # Wrap in Blocks
                destination_floor=2, elevator_state=ElevatorState.MOVING,
                nominal_direction=VerticalDirection.UP, door_open=False,
        ...
        people: list[PersonSnapshot] = [
            PersonSnapshot(
                person_id="person_1", current_floor_num=2, current_vertical_position=Blocks(2.0),  # Wrap in Blocks
                current_horizontal_position=Blocks(5.0), destination_floor_num=1, destination_horizontal_position=Blocks(10),  # Wrap in Blocks
                state=PersonState.WAITING_FOR_ELEVATOR, waiting_time=Time(15.5), mad_fraction=0.2,
                draw_color=(200, 150, 100)
            ),
            PersonSnapshot(
                person_id="person_2", current_floor_num=2, current_vertical_position=Blocks(2.0),  # Wrap in Blocks
                current_horizontal_position=Blocks(8.0), destination_floor_num=1, destination_horizontal_position=Blocks(3),  # Wrap in Blocks
