...
class TestReadyElevatorLogic:
    ...
    def test_ready_elevator_continues_current_direction_when_possible(
        ...
        requests_floor_9 = elevator_bank.get_requests_for_floor(9)
        assert VerticalDirection.UP not in requests_floor_9
    ...
    def test_ready_elevator_chooses_closest_floor_up(
        self, elevator_bank: ElevatorBank, mock_elevator: MagicMock
    ) -> None:
        """Test selection logic for UP direction: choose minimum (closest above)"""
        mock_elevator.current_floor_int = 4
        mock_elevator.nominal_direction = VerticalDirection.UP
        mock_elevator.get_passenger_destinations_in_direction.return_value = []
        
        # Multiple requests going up - should choose the closest (lowest floor number)
        ...
        mock_elevator.set_destination_floor.assert_called_once_with(6)  # Minimum/closest

    def test_ready_elevator_chooses_closest_floor_down(
        self, elevator_bank: ElevatorBank, mock_elevator: MagicMock
    ) -> None:
        """Test selection logic for DOWN direction: choose maximum (closest below)"""
        mock_elevator.current_floor_int = 8
        mock_elevator.nominal_direction = VerticalDirection.DOWN
        mock_elevator.get_passenger_destinations_in_direction.return_value = []
        
        # Multiple requests going down - should choose the closest (highest floor number)
        elevator_bank.request_elevator(3, VerticalDirection.DOWN)
        elevator_bank.request_elevator(5, VerticalDirection.DOWN)
        elevator_bank.request_elevator(2, VerticalDirection.DOWN)
        
        elevator_bank.testing_update_ready_elevator(mock_elevator)
        
        mock_elevator.set_destination_floor.assert_called_once_with(5)  # Maximum/closest
    ...
    def test_ready_elevator_mixed_passengers_and_requests(
        ...
        # Should choose closest passenger destination first (7)
        mock_elevator.set_destination_floor.assert_called_once_with(7)
...
class TestDestinationCollection:
    ...
    def test_collect_destinations_passengers_first(
        ...
        destinations = elevator_bank.testing_collect_destinations(mock_elevator, floor=5, direction=VerticalDirection.UP)
...
class TestRequestClearing:
    ...
    def test_request_cleared_after_destination_set(
        ...
        requests = elevator_bank.get_requests_for_floor(5)
