...
class TestPersonBasics:
    ...
    def test_initial_state(self, person: Person) -> None:
        ...
        assert person.current_floor == 5 # Look at Person factory in conftest.py
        assert person.current_block == 10.0
        assert person.destination_floor == 5  # Same as current initially
    ...
    def test_set_destination_out_of_bounds_clamped(self, person: Person, mock_building: MagicMock) -> None:
        ...
        person.set_destination(dest_floor=15, dest_block=25)  # Both too high
        assert person.destination_floor == mock_building.num_floors  # Clamped to max
        ...
        person.set_destination(dest_floor=-1, dest_block=-5)  # Both too low  
        assert person.destination_floor == 0   # Clamped to min
    ...
    def test_current_block_property(self, person: Person) -> None:
        ...
        person.current_block = 15.5
        assert person.current_block == 15.5
